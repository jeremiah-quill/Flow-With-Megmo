...
function Sidebar() {
    ...
    const { loading: scheduledLoading, data: scheduledData, error: scheduledError, refetch } = useQuery(QUERY_UPCOMING_CLASSES, {fetchPolicy: "network-only"});
		...
		<div className="sidebar">
			<nav className="list-nav">
				<ul className="list-nav-ul">
					<li className="list-nav-item" onClick={() => setSidebarContent(0)}>Available</li>
					<li className="list-nav-item" onClick={() => setSidebarContent(1)}>Registered</li>
					<li className="list-nav-item" onClick={() => setSidebarContent(2)}>Completed</li>
				</ul>
			</nav>
			{sidebarContent === 0 ? (
				<ScheduledClassList
